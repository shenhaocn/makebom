    ...
    With xlSheet.Cells
        ...
        If rngNum Is Nothing Then
            ...
            Select Case bmfAtom(BMF_MountType)
                ...
                Case "S+"
                ItemNum1 = ItemNum1 + 1
                xlsInsert xlSheet, ItemNum1, rngPos1.Row, bmfAtom, OrgEnable, "Î»ºÅ¶ÔÓ¦¶à¸öÎïÁÏ"
                xlSheet.Rows((rngPos1.Row + ItemNum1) & ":" & (rngPos1.Row + ItemNum1)).Font.ColorIndex = 5
        ...
        Else 'ĞèÒªºÏ²¢ ĞŞ¸ÄÊıÁ¿(µÚ5ÁĞ) ºÍÎ»ºÅ(µÚ6ÁĞ) Î»ºÅĞèÒªÖØĞÂÅÅĞò
            ...
            If RealSorted(tmpRefStr, False) = False Then
                xlSheet.Cells(rngNum.Row, 4) = "Î»ºÅÅÅĞòÒì³££¡"
...
Function CreateBOM(bt_value As BomType) As Boolean
    ...
    If bt_value = BOM_µ÷ÊÔ Or bt_value = BOM_ÁìÁÏ Or _
       bt_value = BOM_Éú²ú Or bt_value = BOM_Ô¤ Then
        ...
        Select Case bt_value
        Case BOM_µ÷ÊÔ
            xlSheet.Cells(1, 1) = "µ÷ÊÔBOM³õ¸å"
        Case BOM_ÁìÁÏ
            xlSheet.Cells(1, 1) = "ÁìÁÏBOM³õ¸å"
        Case BOM_Éú²ú
            xlSheet.Cells(1, 1) = "Éú²úBOM³õ¸å"
        Case BOM_Ô¤
            xlSheet.Cells(1, 1) = "Ô¤BOM³õ¸å"
        Case Else
            xlSheet.Cells(1, 1) = "Ô¤BOM³õ¸å"
        End Select
...
Function BomChecker(ExcelBomFilePath As String)
    ...
    Dim intFontSize   As Integer  '×ÖÌå´óĞ¡  µ¥Î»£º°õ
    Dim intRowNum     As Integer  'µ¥´Ê¿í¶È  µ¥Î»£º×Ö·û
    Dim calRowHeight  As Integer  '¼ÆËãËùµÃĞĞ¸ß
    Dim calColWidth   As Integer  '¼ÆËãËùµÃÁĞ¿í
    ...
    'a.µ÷ÕûÎ»ºÅ¶ÔÓ¦µÄÁĞ¿í?ĞĞ¸ßÊ¹Ö®È«²¿ÏÔÊ¾
        ...
        If xlSheet.Cells(j, rngRef.Column) <> "" Then
            ...
            Else
                xlSheet.Cells(j, rngNote.Column) = "Î»ºÅÖØÅÅĞòÒì³££¡"
        ...
        If xlSheet.Cells(j, rngRef.Column) <> "" Then
            ...
            '¼ÆËãËùĞèµÄºÏÊÊµÄĞĞ¸ß ÁĞ¿í
            'Debug.Print "×ÖÌå´óĞ¡Îª£º" & intFontSize & "    µ¥´Ê¿í¶ÈÎª£º" & intWordWidth
            If xlSheet.Cells(j, rngRef.Column).Height / intFontSize < intRowNum Then
                'ĞèÒª¼ÆËãÊÖ¶¯µ÷Õû
                AutoFitFlag = False
                
                'Ê×ÏÈ³¢ÊÔ°´Ä¿Ç°µÄÁĞ¿íµ÷ÕûĞĞ¸ß
            ...
            Else
                ...
                If calColWidth <> xlSheet.Cells(j, rngRef.Column).ColumnWidth Then
                    'ĞĞ¸ßĞèÒªµ÷Õû Í¬Ê±ĞèÒª½«ÆäËûµÄĞĞ×Ô¶¯ÊÊÓ¦ĞĞ¸ß
                    xlSheet.Cells(j, rngRef.Column).ColumnWidth = calColWidth
                    xlSheet.Cells.EntireRow.AutoFit
                End If
                'ÑÕÉ«±ê¼Ç¾­¹ıµ÷ÕûµÄµ¥Ôª¸ñ
                xlSheet.Cells(j, rngRef.Column).Font.ColorIndex = 3 '±ê¼ÇÎªºìÉ« (5ÎªÀ¶É«)
            End If
        End If
           ...
           If InStr(xlSheet.Cells(j, rngRef.Column), "ÓÅÏÈ¼¶") = 0 Then
               ...
               If CInt(xlSheet.Cells(j, rngQty.Column)) <> (UBound(BomAtom) + 1) Then
                  ...
                  xlSheet.Cells(j, rngNote.Column) = xlSheet.Cells(j, rngNote.Column) + _
                                                     "Ôª¼şÊı[" & CInt(xlSheet.Cells(j, rngQty.Column)) & "]" & _
                                                     "Î»ºÅÊı[" & (UBound(BomAtom) + 1) & "]²»ÏàµÈ£¡"
...
'¾«È·¼ÆËãËùĞèµÄĞĞÊı(¸ù¾İÁĞ¿í¼ÆËã£¬ÁĞ¿íµ¥Î»Îª×Ö·û)
...
'¸ù¾İ×ÖÌå´óĞ¡·µ»Ø¶ÔÓ¦µÄÒò×Ó ¼´·µ»Ø£¨×ÖÌå¸ß¶È+ĞĞ¾à£©
